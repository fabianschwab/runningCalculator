<script lang="ts">
	import { cooldownStore, time2Seconds } from '$lib/index.svelte';
	let distance = $derived(time2Seconds(cooldownStore.duration) / time2Seconds(cooldownStore.pace));
	...
	const coolDown = persisted('coolDown', { pace: 0, duration: 0 });

	$effect(() => {
		if ($coolDown.pace !== 0 && $coolDown.duration !== 0 && !isShared) {
			cooldownStore.duration = $coolDown.duration;
			cooldownStore.pace = $coolDown.pace;
		}
	});
	...
	function saveSettings() {
		...
		$coolDown.pace = cooldownStore.pace;
		$coolDown.duration = cooldownStore.duration;
	}
</script>
	...
	<div class="card-body flex gap-3">
		...
		<label class="form-control">
			...
			<input
				step=".1"
				min="0"
				bind:value={cooldownStore.duration}
				type="number"
				pattern="[0-9]+([\.,][0-9]+)?"
		...
		{#if saved}
			<div class="mt-2 flex items-center">
				<svg
					...
					fill="none"
					viewBox="0 0 24 24"
					class="mr-2 h-6 w-6 shrink-0 stroke-success"
				>
					<path
						fill="currentColor"
				...
				<span>Cool-down settings saved!</span>
			</div>
		{:else}
